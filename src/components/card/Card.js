...
function Card(props) {
  ...
  const getCardData = () => {
    Axios.get(
      `https://pro-organiser1.firebaseio.com/boardContents/${boardId}/column/${columnId}/card.json`
    )
      .then((res) => {
  ...
  const handleAddCardClick = (e) => {
    if (
      ...
      if (editDetails) {
        Axios.put(
          `https://pro-organiser1.firebaseio.com/boardContents/${boardId}/column/${columnId}/card/${cardId}.json`,
          {
